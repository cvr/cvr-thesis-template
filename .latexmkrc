# This is a local configuration file for latexmk version 4.11.
...
# In: ftp.eq.uc.pt/software/TeX/support/latexmk/example_rcfiles/pdflatexmkrc

$pdflatex = 'pdflatex --shell-escape %O %S';

# Clean bibtex bbl files
$clean_ext = "${clean_ext} bbl";

# Custom dependency for glossary/glossaries package
# if you make custom glossaries you may have to add items to
# the @cus_dep_list and corresponding sub-routines
add_cus_dep( 'glo', 'gls', 0, 'makeglo2gls' );
sub makeglo2gls {
  system( "makeindex -s \"$_[0].ist\" -t \"$_[0].glg\" -o \"$_[0].gls\" \"$_[0].glo\"" );
}
$clean_ext = "${clean_ext} gls glo";

# The glossaries package, with the [acronym] option,
# produces a .acn file when processed with (xe/pdf)latex and
# then makeindex to process the .acn into .acr and finally
# runs of (xe/pdf)latex to read in the .acr file. Unfortunately
# the glossary package does just the reverse;
# i.e. (xe/pdf)latex processing produces a .acr files and makeindex then
...
# NOTE: the glossary package uses a very different command:
# the <in-ext> and <out-ext>
...
$clean_ext = "${clean_ext} and adx nnd ndx lnd ldx";
...
# note: you may get extras runs if you use
# the .eps extension in the \includgraphics command
